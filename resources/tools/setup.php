<?php
  ...
  /*
     ...
     1) You installed Application, and pointed your browser to the setup.php
        script, and were expecting the Application setup screen to come up.
        ...
        administrator. If you can't get it to work, join the Application
   ...
   *
   * This script can be used to install the Application database, and/or upgrade
  ...
  function displayErrors($errors, $fixablebyuser=true)
  {
    ...
    echo '<html>
            ...
            <body bgcolor="#ffffff">
              ...
              <br>
              <br>We\'re very sorry, but this server is unable to run Application,
    ...
    if ($fixablebyuser)
    {
      echo '  If you can\'t get Application to work after following the above instructions, please visit the <a href="http://www.achievo.org/forum" target="_new">Application forums</a> and we might be able to help you.';
    }
    echo '  </body>
          </html>';
  }
  ...
  {
    $errors[] = "You are using PHP version ".phpversion().", but you need at least php version ".REQUIRED_PHP." to run Application.
                 <br>Please upgrade your installation or consult your system administrator.";
  }

  // Some versions of PHP contain bugs and behave badly when running Application.
  // Therefore, we check for known buggy versions.
  $version = phpversion();
  if ($version=="4.1.2")
  {
    $errors[] = "The PHP version you are using ($version) is known to have some
                 bugs that prevent Application from running properly.
  ...
  else
  {
    ...
    {
      $errors[] = "The minimal memory limit for running Application is ".ACHIEVO_MIN_MEM."M.
  ...
  // Application relies on some PHP settings. In future versions, Application should not
...
/*
  ...
  if (!$register_globals)
  {
    $errors[] = "The <b>register_globals</b> setting in <b>$inilocation</b> is set to Off.
                 <br>Currently, Application will only function with this setting turned on.
  ...
  */
  ...
  // Application 1.1 requires a writable temp dir. We must check if we can write.
	  $fp = @fopen("atktmp/compiled/tpl/setuptest", "w");
  if ($fp==FALSE)
  {
    $errors[] = "The Application temporary directory is not writable by the webserver.
                 <br>Please check the permissions of the atktmp/ directory (and its subdirectories).
                 <br><br>On Linux/Unix you can run the command 'chown -R <i>username</i> atktmp' to make the directory
  ...
  {
    fclose($fp);
    unlink("atktmp/compiled/tpl/setuptest");
  }
  ...
  //Validate if called from an authorizes IP sanotto
  $ip=$_SERVER['REMOTE_ADDR'];
  $ips_allowed=split(",",atkconfig("setup_allowed_ips"));
  
  if (!atk_in_array($ip, $ips_allowed) ) 
  {
  	$ERrors[] = "Setup can only be called from authorized IP addresses, consult your sysadmin.Your IP:".$ip."</b>";
  }	
  // If there are any errors when we reach this point, it's no use continuing because
  ...
  {
    $errors[] = "Application will not run for a (yet) unkown reason. The install-script will dump
                 some information below.
                 <br>Please e-mail this entire page to bugs@achievo.org for analysis, so we might
                 be able to help you, or fix Application to work with your setup.
